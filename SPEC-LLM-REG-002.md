...
Document ID: SPEC-LLM-REG-002Version: 1.1-draftDate: 2026-06-13Author: Florian FriedrichStatus: Draft for Review
...
- LLM usage: Yes — runtime LLM via Requesty/deepseek-v4-pro (Sections 5.1, 5.1 LLM Configuration)
...
- Max parallel requests: 5 concurrent (across all providers)
...
> **[IMPL]** A ParserRegistry maps provider_id to ParserClass. Simple providers need zero code - just config.
...
4. Add custom parser only if generic is insufficient
5. Create golden output (expected_output.json)
6. Run tests: pytest tests/ -k <id>
7. Document quirks in the parser module docstring

### 13.5 Key Implementation Decisions

1. **State persistence:** MODELS.json as canonical state, or separate internal DB?
2. **Sync vs. async:** Fully async with bounded concurrency, or sequential?
3. **Model-key collision:** Sanitise model_id with slashes/dots into safe keys?
4. **Failure isolation:** Abort on single provider failure, or continue?
5. **LLM fallback:** Cache by content hash, temperature=0, configurable call cap?
6. **Multi-API expansion:** One entry per (model, api_type) for providers with multiple APIs?
7. **Generic vs custom parser threshold:** What completeness metric gates custom code?
8. **Scheduling:** One-shot CLI by external cron, or embedded scheduler?
...
API endpoint: https://api.requesty.ai/v1/models (OpenAI-compatible, requires key)
Note: Verification needed — API docs may be sparse
